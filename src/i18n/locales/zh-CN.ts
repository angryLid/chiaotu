...
export const zhCN = {
	...
	ruleSets: {
		title: "规则集",
		description:
			"把域名 / IP 匹配规则打包成一个集合，通过公开链接分发；生成配置时以 rule-providers 引用，客户端会按自己的周期拉取最新内容。",
		new: "+ 新建规则集",
		refresh: "刷新",
		refreshing: "刷新中…",
		empty: "还没有规则集，点击右上角「新建规则集」创建第一个。",
		expand: "展开规则集「{{name}}」",
		collapse: "收起规则集「{{name}}」",
		itemCount: "{{count}} 条规则",
		limitReached: "最多可创建 {{max}} 个规则集，已达上限",
		createTitle: "新建规则集",
		editTitle: "编辑规则集「{{name}}」",
		edit: "编辑",
		delete: "删除",
		create: "创建",
		save: "保存",
		submitting: "提交中…",
		deleteConfirm:
			"确定删除规则集「{{name}}」吗？其下所有规则也会被删除，分发链接立即失效。",
		field: {
			name: "规则集名称",
		...
		link: {
			title: "分发链接",
			hint: "链接即凭证：拿到链接的人无需令牌即可读取本规则集内容。",
			copy: "复制链接",
			copied: "已复制",
			rotate: "重置链接",
			rotating: "重置中…",
			rotateConfirm:
				"重置后旧链接立即失效，已经引用旧链接的配置需要重新生成。确定继续吗？",
			providerKey: "rule-providers 键名：{{key}}",
		},
		items: {
			title: "规则条目",
			empty: "还没有规则，点击「导入规则」粘贴添加。",
			enabled: "{{payload}} 开关",
			remove: "删除",
			removeConfirm: "确定删除规则「{{payload}}」吗？",
			hint: "类型与内容不可修改；要改写请删除后重新导入。",
		},
		import: {
			open: "导入规则",
			title: "导入规则到「{{name}}」",
			...
			hint: "每行一条，只读前 50 行。可直接写「类型,内容」；不写类型时自动判断（裸域名按 DOMAIN-SUFFIX，带 * 或 ? 按 DOMAIN-WILDCARD，IP / CIDR 按 IP-CIDR）。空行与 # 注释会跳过。",
			preview: "预览导入",
			report: "导入报告",
			accepted: "已识别：{{count}} 条",
			skipped: "已跳过：{{count}} 行",
